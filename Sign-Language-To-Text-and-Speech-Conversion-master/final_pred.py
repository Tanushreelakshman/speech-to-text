...
class Application:
    ...
    def __init__(self):
        ...
        self.speak_engine = TextToSpeech(rate=100, voice_index=0)
        ...
        # Configure main window (smaller, so all controls fit on screen)
        self.root = tk.Tk()
        self.root.title("Sign Language To Text Conversion")
        self.root.protocol('WM_DELETE_WINDOW', self.destructor)
        # Use a compact window size
        self.root.geometry("960x600")

        # Main video panel (left)
        self.panel = tk.Label(self.root)
        self.panel.place(x=10, y=50, width=480, height=360)

        # Hand preview / processed image (right)
        self.panel2 = tk.Label(self.root)
        self.panel2.place(x=500, y=50, width=320, height=240)

        # Title
        self.T = tk.Label(self.root)
        self.T.place(x=10, y=10)
        self.T.config(text="Sign Language To Text Conversion", font=("Courier", 18, "bold"))

        # Current symbol display (near hand preview)
        self.T1 = tk.Label(self.root)
        self.T1.place(x=500, y=300)
        self.T1.config(text="Character :", font=("Courier", 14, "bold"))

        self.panel3 = tk.Label(self.root)
        self.panel3.place(x=650, y=300)

        # Sentence label and display (below video)
        self.T3 = tk.Label(self.root)
        self.T3.place(x=10, y=420)
        self.T3.config(text="Sentence :", font=("Courier", 14, "bold"))

        self.panel5 = tk.Label(self.root)
        self.panel5.place(x=110, y=420)

        # Suggestions label
        self.T4 = tk.Label(self.root)
        self.T4.place(x=10, y=460)
        self.T4.config(text="Suggestions :", fg="red", font=("Courier", 12, "bold"))

        # Suggestion buttons (arranged in a row)
        self.b1 = tk.Button(self.root)
        self.b1.place(x=10, y=500, width=220, height=40)

        self.b2 = tk.Button(self.root)
        self.b2.place(x=240, y=500, width=220, height=40)

        self.b3 = tk.Button(self.root)
        self.b3.place(x=470, y=500, width=220, height=40)

        self.b4 = tk.Button(self.root)
        self.b4.place(x=700, y=500, width=220, height=40)

        # Clear and Speak buttons (bottom-right corner)
        self.clear = tk.Button(self.root)
        self.clear.place(x=700, y=550, width=100, height=36)
        self.clear.config(text="Clear", font=("Courier", 12), command=self.clear_fun)

        self.speak = tk.Button(self.root)
        self.speak.place(x=820, y=550, width=120, height=36)
        self.speak.config(text="Speak", font=("Courier", 12), command=self.speak_fun)
    ...
    def video_loop(self):
        try:
                    ...
                    if image is not None and image.size > 0:
                        try:
                            ...
                            if handz and len(handz) > 0 and handz[0]:
                                ...
                                self.b1.config(text=self.word1, font=("Courier", 12), wraplength=200, command=self.action1)
                                self.b2.config(text=self.word2, font=("Courier", 12), wraplength=200,  command=self.action2)
                                self.b3.config(text=self.word3, font=("Courier", 12), wraplength=200,  command=self.action3)
                                self.b4.config(text=self.word4, font=("Courier", 12), wraplength=200,  command=self.action4)
                        except Exception as e:
                            pass  # Skip hand detection if it fails

                self.panel5.config(text=self.str, font=("Courier", 14), wraplength=700)
        except Exception:
            ...
            try:
                ...
                pass  # Skip exception handler if it fails
    ...
    def speak_fun(self):
        # Use non-blocking speak() for real-time responsiveness
        # For immediate speech (blocking), use speak_blocking(self.str)
        self.speak_engine.speak(self.str)
